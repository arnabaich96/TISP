...
final_logistic = function(lambda1,eta=1/nrow(data_train$y))
{
  w_init = as.matrix(rep(0, ncol(data_train$x)))
  r1 = logistic(100, data_train, data_test, lambda=lambda1
                ,w_init = as.matrix(rep(0, ncol(data_train$x))))
 
 table = data.frame(lambda1,r1$num_feature,r1$train_miss,r1$test_miss)
  colnames(table) = c("lambda","Feature","Miss.Train","Miss.Test")

  return(table)
}
r = logistic(100, data_train, data_test, lambda =4251833)
r$num_feature
r$train_miss
r$test_miss
...
ggplot(D,aes(x= Feature))+
  geom_line(aes(y = Miss.Train,color = "Training"))+
  geom_line(aes(y = Miss.Test,color = "Testing"))+
  scale_x_continuous(limits=c(0,500),breaks = c(10,30,98,301,500))+
  ylim(0,0.2) + ylab('Misclassification Error')+ xlab('Number of Feature')

# Missclassification Error Plot
Miss_plot = function(n_iter,lambda=2265200,eta,data)
{
  ...
  miss.plot = ggplot(data, aes(x = iteration)) + 
    ...
    annotate("text", x=75, y=0.06, label= "Feature = 100",
             col="blue", size=5)
  return(miss.plot)
}
Miss_plot(100,2265200,eta=1/nrow(data_train$y),data_train)

# Roc Plot
 Roc_plot = function(n_iter,lambda=1219900,eta,train,test)
 {
...
#                 interval = c(1000000,600000))$root})
